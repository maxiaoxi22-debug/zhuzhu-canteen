...
export default function CompletedWishlistPage({ onClose, onOpenRecipe }: CompletedWishlistPageProps) {
            ...
            {items.map((item) => (
              <article key={item.id} className="surface-card rounded-2xl p-4">
                <div className="flex items-center gap-3">
                  <span className="grid h-12 w-12 flex-none place-items-center rounded-xl bg-[#e8f4e2] text-2xl">✓</span>
                ...
                {item.recipeId && <button type="button" onClick={() => onOpenRecipe(item.recipeId!)} className="mt-3 min-h-11 w-full rounded-xl bg-[var(--cocoa)] text-xs font-bold text-white">再看一次菜谱</button>}
              </article>
            ))}
          </div>
        )}
